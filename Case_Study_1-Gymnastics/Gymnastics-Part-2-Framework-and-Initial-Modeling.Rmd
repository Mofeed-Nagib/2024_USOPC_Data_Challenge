...
1. Think about the 3-4 main steps that need to be done (for example data cleaning, modeling, etc., or something more specific to this project) and the details of what needs to be done in each of those steps.  
```{r}
# prep data
1. clean up names and dates. Make sure name or date format
2. Add columns that help with the model


# for model
1. Predict the scores and var of each athlete
2. After done predicting the scores separate step for weighting the scores
3. Final output. Play around with it, add own constraints, chose players.

```

```{r}

d = read.csv("/Users/bobolo/Downloads/data_2017_2021 (1).csv")




```
2. For each of those main steps, think about how to break the work into 3 parts that can be worked on in parallel by the 3 members of your group. 
3. Create a `main.r` script that looks like this that will serve as a sort of outline for all of your scripts or functions
