...
all_male_athletes <- rbind(men_athletes[,c('fullname', 'country', 'flag_team')],
                           ...
                           data.frame(fullname = sub_us_men, country = 'USA',
                                      flag_team = 1))
...
ls_qual_competitors <- purrr::map(mens_apparatus, 
...
for (trial in 1:trials) {
  qual_scores <- qual_scores %>% 
                 mutate(!!as.name(paste0("aa_", trial)) := get(paste0("fx_", trial)) +
                                                           ...
                                                           get(paste0("hb_", trial)) +
                                                           get(paste0("pb_", trial)) +
                                                           get(paste0("sr_", trial)) +
                                                           get(paste0("ph_", trial)))
...
for (team_combo in 1:men_team_combos) {
  ...
  simulated_scores <- qual_scores %>%
                      filter(country != 'USA' | fullname %in% as.character(df_male_us_teams[team_combo,1:5]))
  ...
  ls_medal_winners <- foreach(trial = 1:trials) %dopar% {
    ...
    out_us_results <- plyr::rbind.fill(fx_final, vt_final, hb_final, pb_final, sr_final, 
                                       ph_final, aa_final, out_team_winners)
