...
lm2 <- lm(score ~ apparatus + fullname + round, data = d)
lmer2 <- lmer(score ~ (1 + apparatus|fullname) + round, data = d)
...
for(j in 1:k){
  ...
  d[test.rows,]$lm1 <- predict(lm1, newdata=d[test.rows,], type='response', allow.new.levels=T)
